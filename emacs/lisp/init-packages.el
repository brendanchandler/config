...
;; clipetty forwards the kill ring to the system clipboard via OSC 52,
;; so M-w works from `emacs -nw` inside tmux (and over SSH). Terminal only;
;; the GUI already syncs with the X clipboard.
